...
import {ISaleWare} from '../../modules/apps/reports/sale/models/sale_model'
import {SALE_WARE_COLUMNS} from '../../modules/apps/reports/sale/types/Columns'
...
import { HeaderWare } from '../../modules/apps/reports/sale/components/HeaderWare'

const SaleWare: React.FC = () => {
  const intl = useIntl()
  const {saleWareItems} = useContext(FilterContext)  

  return (
    <>
      <PageTitle breadcrumbs={[]}>{intl.formatMessage({id: 'MENU.SALE_WARE'})}</PageTitle>
      <ItemsContainer items={saleWareItems} />
    </>
  )
}
...
const ItemsContainer = ({items}: {items: any}) => {
  const intl = useIntl()
  const columns = useMemo(() => SALE_WARE_COLUMNS, [])
  ...
  function exportPDF() {
    ...
    const head = [
      [
        intl.formatMessage({id: 'WARE_CODE'}),
        intl.formatMessage({id: 'WARE_NAME'}),
    ...
    const data = items.map((item: ISaleWare) => {
  ...
  function exportCSV() {
    ...
    let str = `${intl.formatMessage({id: 'WARE_CODE'})};${intl.formatMessage({
      id: 'WARE_NAME',
